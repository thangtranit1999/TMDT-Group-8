...
class Xoo_Helper{
	...
	public function get_theme_templates( $scan_woocommerce = false ) {
		...
		foreach ( $scanned_files as $lookInDir => $files ) {
			foreach ( $files as $file ) {
				...
				} elseif ( file_exists( get_stylesheet_directory() . '/templates/' . $this->slug .'/'. $file ) ) {
					$theme_file = get_stylesheet_directory() . '/templates/' . $this->slug .'/'. $file;
				} elseif ( file_exists( get_template_directory() . '/templates/' . $file ) ) {
					$theme_file = get_template_directory() . '/templates/' . $file;
				} elseif ( file_exists( get_template_directory() . '/templates/' . $this->slug .'/'. $basename ) ) {
					$theme_file = get_template_directory() . '/templates/' . $this->slug .'/'. $basename;
	...
	public function get_outdated_section(){
		...
		<div class="xoo-outdatedtemplates">
			<?php if( $odTempData['has_outdated'] === "yes" ): ?>
				<span><?php _e( "You're using outdated version of templates, please fetch a new copy from the plugin templates folder", $this->slug ); ?></span>
				<ul>
					<?php
			...
			<?php else: ?>
				<div><?php _e( 'Templates Status', $this->slug ); ?>
				<span class="dashicons dashicons-yes-alt" style="font-size: 14px;color: #008000;line-height: 1.3;"></span>
				<a href="https://docs.xootix.com/<?php echo $this->slug; ?>" target="_blank"><?php _e( 'How to override?', $this->slug ); ?></a>
				</div>
			<?php endif; ?>
			<span>Last checked: <?php echo get_date_from_gmt( date( 'Y-m-d H:i:s', $odTempData['last_scanned'] ) ); ?></span>
			<a href="<?php echo esc_url( add_query_arg( 'scan_templates', 'yes' ) ); ?>">Check again</a>
		</div>
		<?php
		return ob_get_clean();
	}
	...
	public function time_to_update_theme_templates_data(){
		...
		if(  ( ( time() - $tempData['last_scanned'] ) > ( 86400 * 2 ) ) || isset( $_GET['scan_templates'] ) ){
			$this->update_theme_templates_data();
			wp_safe_redirect( remove_query_arg( 'scan_templates' ) );
			die();
		}
	}
